import { IGmcBody } from "../../interfaces/IGmcBody";

export const CommissionerDetails:IGmcBody = {
    name:'Sri P Sreenivasulu,I.A.S',
    designation:'Commissioner',
    email:'mc.guntur@cdma.gov.in',
    mobile:'9849908385',
    description:'It Is Our Endeavour To Bring The Best Of The Muncipal Services At The Doorstep Of The Citizens Of Guntur And To Serve With 24x7 And 360 Degree Approach.',
    image:'/assets/gmc-body/comissioner.jpg',
};

export const MayorDetails:IGmcBody = {
    name:'Sri Kavati Sivanaga Manohar Naidu',
    designation:'Mayor',
    email:'manohar.guntur@cdma@gov.in',
    mobile:'9849488999',
    description:'Building a sustainable GUNTUR in all spheres will need participation and engagement of all  stake holders including the private sector and the civil society especially the poor and marginalized',
    image:'/assets/gmc-body/mayor.jpg',
};

export const DeputyMayorDetails:IGmcBody[] =[{
    name:'Sri Vanama Bala Vajra Babu',
    designation:'Deputy Mayor-I',
    mobile:'9553434777',
    description:'Building a sustainable GUNTUR in all spheres will need participation and engagement of all  stake holders including the private sector and the civil society especially the poor and marginalized',
    image:'/assets/gmc-body/deputy_mayor1.jpeg',
},
{
    name:'Smt Shaik Sajeela',
    designation:'Deputy Mayor-II',
    mobile:'9849671212',
    description:'Building a sustainable GUNTUR in all spheres will need participation and engagement of all  stake holders including the private sector and the civil society especially the poor and marginalized',
    image:'/assets/gmc-body/deputy_mayor2.jpg',
},
];

export const AdditionalCommissionerDetails:IGmcBody = {
    name:'Sri C Obulesu',
    designation:'Additional Commissioner',
    email:'ac.gunturcorporation@gmail.com',
    mobile:'9849908396',
    description:'It Is Our Endeavour To Bring The Best Of The Muncipal Services At The Doorstep Of The Citizens Of Guntur And To Serve With 24x7 And 360 Degree Approach.',
    image:'/assets/gmc-body/additional_commissioner.jpg',
};